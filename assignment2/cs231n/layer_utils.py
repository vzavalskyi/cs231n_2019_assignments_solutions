...
def affine_nm_relu_forward(x, w, b, gamma, beta, param, norm='batchnorm'):
    """
    Convenience layer that perorms an affine transform followed by
    batch or layernorm normalization and a ReLU
    ...
    a, fc_cache = affine_forward(x, w, b)
    if norm == 'batchnorm':
        norm_out, norm_cache = batchnorm_forward(a, gamma, beta, param)
    elif norm == 'layernorm':
        norm_out, norm_cache = layernorm_forward(a, gamma, beta, param)
    out, relu_cache = relu_forward(norm_out)
    cache = (fc_cache, norm_cache, relu_cache)
    return out, cache


def affine_nm_relu_backward(dout, cache, norm='batchnorm'):
    """
